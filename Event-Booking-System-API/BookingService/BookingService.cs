    ...
    public class BookingService : IBookingService
    {
        ...
        public async Task<(AddBookingResponse?, string?)> AddBookingAsync(BookingRequest bookingDto, string userId)
        {
            try
            {
                ...
                // Optional: Validate UserId if necessary (e.g., check if user exists)
                // For now, we assume userId is valid and comes from an authenticated context.

                var bookingEntity = bookingDto.ToBooking(userId); // Use userId as per updated mapper
